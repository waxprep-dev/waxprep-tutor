...
def evaluate_theory_answer(answer_text: str, marking_scheme: Dict[str, Any]) -> Tuple[float, str, Dict[str, Any]]:
    """
    Evaluate a student's theory answer against a marking scheme.
    
    Returns: (score, feedback, detailed_breakdown)
    """
    ...
    for criterion in criteria:
        ...
        # Check if key phrases are present in answer
        phrases_found = 0
        ...
        if key_phrases:
            phrase_ratio = phrases_found / len(key_phrases)
        else:
            phrase_ratio = 0.0
    ...
    if total_marks > 0:
        overall_score = round((earned_marks / total_marks) * 100, 1)
    else:
        overall_score = 0.0
    
    # Build feedback
    feedback_parts = []
    feedback_parts.append(f"Score: {earned_marks}/{total_marks} marks ({overall_score}%)")
    
    for item in breakdown:
        if item["status"] == "full":
            feedback_parts.append(f"✅ {item['type'].capitalize()}: Got it! ({item['marks_earned']}/{item['marks_possible']})")
        elif item["status"] == "partial":
            feedback_parts.append(f"⚠️ {item['type'].capitalize()}: Partial — you mentioned {item['phrases_found']}/{item['phrases_total']} key points. ({item['marks_earned']}/{item['marks_possible']})")
        else:
            feedback_parts.append(f"❌ {item['type'].capitalize()}: Missing — need to mention: {', '.join(key_phrases[:3])}. (0/{item['marks_possible']})")
    
    # Add improvement tip
    missing = [b for b in breakdown if b["status"] == "missing"]
    if missing:
        feedback_parts.append(f"\n💡 Focus on: {missing[0]['type'].capitalize()} — this is where most marks are.")
