/**
 * WaxPrep Embedding Service v3.0
...
export class EmbeddingService {
  ...
  private cacheTTL: number = 86400000;
  ...
  static fromEnvironment(): EmbeddingService {
    if (process.env.OPENAI_API_KEY) {
      const model = (process.env.OPENAI_EMBEDDING_MODEL as 'text-embedding-3-small' | 'text-embedding-3-large' | 'text-embedding-ada-002') || 'text-embedding-3-small';
      return new EmbeddingService(new OpenAIEmbeddingProvider(process.env.OPENAI_API_KEY, model));
    }

    if (process.env.COHERE_API_KEY) {
      const model = (process.env.COHERE_EMBEDDING_MODEL as 'embed-english-v3.0' | 'embed-multilingual-v3.0') || 'embed-english-v3.0';
