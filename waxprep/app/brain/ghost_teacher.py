"""
================================================================================
GHOST TEACHER v4.0 - INDEPENDENT STUDY MODE
================================================================================
Unchanged from v3.0 - solid implementation.
================================================================================
"""
...
def parse_study_intent(message: str) -> Optional[Tuple[str, int]]:
    ...
    # v4.0 FIX: Smarter topic extraction
    import re

    # Extract duration first
    duration = 20
    duration_match = re.search(r'(\d+)\s*(?:minutes|minute|min)', msg_lower)
    if duration_match:
        duration = int(duration_match.group(1))
        duration = max(5, min(60, duration))

    # Find which study phrase matched and remove everything before it
    topic = ""
    matched_phrase = ""
    ...
    if matched_phrase:
        # Get everything after the study phrase
        idx = msg_lower.index(matched_phrase) + len(matched_phrase)
        remainder = msg_lower[idx:].strip()

        # Remove duration from remainder (longer matches first!)
        remainder = re.sub(r'\d+\s*(?:minutes|minute|min)', '', remainder).strip()

        # Remove "for" that was before the duration (e.g., "study for 20 minutes")
        remainder = re.sub(r'\bfor\b', '', remainder).strip()

        # Remove common connecting words at start
        remainder = re.sub(r'^(about|on|regarding)\s+', '', remainder).strip()

        # Clean up trailing punctuation and words
...
def evaluate_ghost_answer(answer: str, question_type: str, concept: str) -> Tuple[float, str]:
    ...
    base_score = min(1.0, word_count / 20)
    understanding_bonus = min(0.4, understanding_score * 0.1)
    confusion_penalty = min(0.3, confusion_score * 0.1)

    score = max(0.0, min(1.0, base_score + understanding_bonus - confusion_penalty))
